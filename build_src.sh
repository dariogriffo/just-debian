...
if [ -z "$JUST_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <just_version> <build_version>"
    echo "Example: $0 1.56.0 1"
    exit 1
fi

PACKAGE_NAME="just"
ORIG_TARBALL="${PACKAGE_NAME}_${JUST_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${JUST_VERSION}"

echo "Creating Debian/Ubuntu source packages for just ${JUST_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/casey/just/archive/refs/tags/${JUST_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${JUST_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as just-1.x.y/ which matches our BUILD_DIR
...
just (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${JUST_VERSION}.
