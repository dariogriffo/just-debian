...
if [ -z "$JUST_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    ...
    echo "Supported architectures: amd64, arm64, armhf, riscv64, loong64, all"
    exit 1
fi

BUILD_DATE="$(date -R)"

# Function to map Ubuntu architecture to the just release target triple
get_just_target() {
    local arch=$1
    case "$arch" in
...
build_architecture() {
    ...
    if [ -z "$target" ]; then
        ...
        echo "Supported architectures: amd64, arm64, armhf, riscv64, loong64"
        return 1
    fi
    ...
    # riscv64 and loong64 are only available from noble (24.04) onwards.
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$JUST_VERSION-${BUILD_VERSION}~${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "just-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "just-ubuntu-$dist-$build_arch")"
